...
sequenceDiagram
    ...
    G->>DB: 验证用户身份
    G->>MQ: 发布 GAME_CREATION_REQUESTED
    MQ->>CA: 消费消息
    CA->>CA: AI生成游戏世界
    CA->>DB: 保存游戏数据
    CA->>MQ: 发布 GAME_CREATION_COMPLETED
    MQ->>G: 推送完成事件
    G->>F: WebSocket通知
    F->>U: 显示游戏创建成功
```
...
sequenceDiagram
    ...
    G->>DB: 验证游戏状态
    G->>MQ: 发布 PLAYER_ACTION_SUBMITTED
    MQ->>LA: 消费逻辑推理任务
    LA->>LA: AI逻辑推理
    LA->>DB: 更新游戏状态
    LA->>MQ: 发布 LOGIC_PROCESSING_COMPLETE
    MQ->>NA: 消费叙事生成任务
    NA->>NA: AI生成叙事
    NA->>MQ: 发布 NARRATIVE_GENERATION_COMPLETED
    MQ->>G: 推送结果事件
    G->>F: WebSocket实时推送
    F->>P: 显示AI生成内容
```
