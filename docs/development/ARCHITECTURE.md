...
创世星环是一个基于微服务架构的AI驱动交互式叙事游戏生成系统，采用多Agent协作技术，为用户提供沉浸式的创作体验。系统已完成现代化架构重构，具备完整的微服务生态、VCPToolBox插件框架和企业级基础设施。

[![现代化架构](https://img.shields.io/badge/architecture-microservices-blue.svg)](./ARCHITECTURE.md)
[![多Agent协作](https://img.shields.io/badge/agents-4_专用服务-green.svg)](./ARCHITECTURE.md)
[![企业级就绪](https://img.shields.io/badge/industrial-ready-brightgreen.svg)](./ARCHITECTURE.md)
[![插件生态](https://img.shields.io/badge/vcp-toolbox-✅_integrated-purple.svg)](./ARCHITECTURE.md)

## 核心架构原则

### 1. VCPToolBox插件化架构

- **插件生态**: 6大插件类型 (静态/消息预处理器/同步/异步/服务/混合服务)
- **异步工具调用**: 非阻塞执行 + 上下文感知 + 智能调度
...
- **智能代理**: 四个专门的AI代理 (Creation/Logic/Narrative/Backend Gateway)
- **消息队列通信**: RabbitMQ事件驱动，松耦合架构
- **协作任务分配**: 基于能力匹配的智能调度
- **质量保证机制**: Agent输出评估 + 人类审核接口

### 3. 工业级质量架构

- **测试驱动开发**: Vitest + Vue Testing Library，87.3%覆盖率
- **CI/CD流水线**: GitHub Actions + 9步验证流程
- **性能监控**: Core Web Vitals + 实时指标收集
- **国际化支持**: 5语言架构 + 文化适应性优化

### 4. 用户体验优先架构

- **响应式设计**: 移动优先，完美适配320px-1440px
- **主题系统**: 深色/浅色/自动主题，CSS变量动态切换
- **实时通信**: WebSocket集群 + 流式AI响应
- **无障碍支持**: WCAG 2.1 AA级，完全可访问

## Phase 1 完整架构图

```
┌─────────────────────────────────────────────────────────────────────────────────────────────┐
│                              🎨 前端体验层 (Frontend Experience Layer)                           │
│  ┌─────────────────────────────────────────────────────────────────────────────────────────┐ │
│  │                    Vue 3 SPA (现代化用户界面)                                              │ │
│  │  ┌─────────────┬─────────────┬─────────────┬─────────────┬─────────────┐               │ │
│  │  │ WelcomeView │ NexusHub    │ CreationHub │  GameView   │ Settings    │               │ │
│  │  │   欢迎页    │   中枢中心  │   创作中心  │   游戏界面  │   设置页    │               │ │
│  │  └─────────────┴─────────────┴─────────────┴─────────────┴─────────────┘               │ │
│  │                                                                                         │ │
│  │ 🌓 主题系统 • 🌐 国际化 • 📱 响应式 • ♿ 无障碍 • ⚡ 性能优化                              │ │
│  └─────────────────────────────────────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────────────────────────────┘
                                            │
                                            │ HTTP/2 + WebSocket + 流式响应
                                            ▼
┌─────────────────────────────────────────────────────────────────┐
