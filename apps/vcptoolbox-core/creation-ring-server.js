...
class CreationRingServer {
  ...
  async loadExistingData() {
    try {
      ...
      logger.info(`📊 加载数据完成: ${this.storyDatabase.size} 故事, ${this.characterDatabase.size} 角色, ${this.worldDatabase.size} 世界`);
  ...
  async updateStory(req, res) {
    ...
    // 保存到文件
    await fs.writeFile(
      path.join(STORY_DATABASE_PATH, `${id}.json`),
      JSON.stringify(story, null, 2)
    );
  ...
  async backupDatabases() {
    try {
      const backupDir = path.join(__dirname, 'data', 'backups', new Date().toISOString().split('T')[0]);
