...
class WorldBuilder {
   ...
   * 创建新世界
   */
  async createWorld(parameters, _sessionId) {
    const {
      name,
      type = 'fantasy',
      scale = 'continent',
   ...
   * 扩展世界
   */
  async expandWorld(parameters, _sessionId) {
    const {
      worldId,
      currentWorld,
      expansionArea,
   ...
   * 创建具体地点
   */
  async createLocation(parameters, _sessionId) {
    const {
      world,
      name,
      type = 'city',
   ...
   * 构建文化体系
   */
  async buildCulture(parameters, _sessionId) {
    const {
      world,
      cultureName,
   ...
   * 创建世界历史
   */
  async createHistory(parameters, _sessionId) {
    const {
      world,
      timeline = [],
      keyEvents = [],
   ...
   * 定义世界规则
   */
  async defineRules(parameters, _sessionId) {
    const {
      world,
      ruleType,
      physicalRules = [],
  ...
  buildWorldCreationPrompt(options) {
    const { name, type, scale, era, themes, uniqueElements, conflicts, magic } = options
...
请创建富有创意和逻辑一致的世界观。`
  ...
  buildWorldExpansionPrompt(options) {
    const { currentWorld, expansionArea, expansionType, integrationRequirements } = options
...
请进行有创意的世界扩展。`
  ...
  buildLocationCreationPrompt(options) {
    const { world, name, type, importance, features, inhabitants, significance } = options
...
请创建生动、立体的地点描述。`
  ...
  buildCultureCreationPrompt(options) {
    const { world, cultureName, aspects, influences, conflicts, uniqueTraits } = options
...
请创建丰富、多层次的文化体系。`
  ...
  buildHistoryCreationPrompt(options) {
    const { world, timeline, keyEvents, historicalFigures, majorConflicts, technologicalProgress } =
      options
...
请创建逻辑连贯的历史叙述。`
  ...
  buildRulesDefinitionPrompt(options) {
    const { world, ruleType, physicalRules, magicalRules, socialRules, exceptions, consequences } =
      options
...
请创建清晰、一致的规则体系。`
  ...
  compareWorldStates(_original, _expanded) {
    return {
