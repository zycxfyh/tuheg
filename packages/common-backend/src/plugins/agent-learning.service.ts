...
export class AgentLearningService {
  ...
  async calculateAgentPerformance(
    ...
    const averageResponseTime = completedTasksWithTiming.length > 0
      ? completedTasksWithTiming.reduce((sum, task) => {
        ...
        }, 0) / completedTasksWithTiming.length / (1000 * 60) // 转换为分钟
      : 0;
    ...
    const collaborationScore = totalCollaborations > 0
      ? successfulCollaborations / totalCollaborations
      : 0;

    // 计算学习适应性
    const learningData = await this.getAgentLearningHistory(agentId, undefined, 100);
    const recentLearning = learningData.filter(
      l => l.appliedAt >= periodStart
    );
    const learningAdaptability = recentLearning.length > 0
  ...
  async generatePerformanceReport(agentId: string): Promise<any> {
    ...
    // 生成改进建议
    const recommendations = this.generateImprovementRecommendations(currentMetrics, learningPatterns);
  ...
  async applyOptimization(agentId: string, optimizationType: string): Promise<boolean> {
    ...
    let updatedCapabilities = [...agent.capabilities];
