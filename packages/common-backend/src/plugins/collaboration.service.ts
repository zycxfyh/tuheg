...
export class CollaborationService {
  ...
  async getCollaborationResult(collaborationId: string): Promise<CollaborationResult> {
    ...
    const duration = collaboration.completedAt && collaboration.createdAt
    ...
    return {
      ...
      participants: [
        ...
        ...collaboration.participants.map(p => p.id)
      ],
  ...
  async getCollaborationStats(collaborationId: string): Promise<any> {
    ...
    return {
      ...
      tasks: Object.fromEntries(
        taskStats.map(s => [s.status, s._count.status])
      ),
  ...
  private async executeLeaderDrivenStrategy(collaboration: AgentCollaboration): Promise<void> {
    ...
    const leaderTasks = collaboration.tasks.filter(t => t.assignedAgentId === collaboration.leaderId);
   ...
   * 为任务找到最佳Agent
   */
  private async findBestAgentForTask(taskId: string, collaboration: AgentCollaboration): Promise<any> {
    ...
    const agentScores = await Promise.all(
      participants.map(async (agent) => {
        ...
        // 检查Agent是否有相关能力
        const hasCapabilities = agent.capabilities?.some((cap: any) =>
          task.type.toLowerCase().includes(cap.id)
        ) || false;
  ...
  private async resolveTaskAssignmentConflict(collaboration: AgentCollaboration, conflict: any): Promise<string> {
    // 重新分配任务给其他Agent
    return 'task_reassigned';
  }

  private async resolveDecisionMakingConflict(collaboration: AgentCollaboration, conflict: any): Promise<string> {
    // 领导者做出最终决定
    return 'leader_decision';
  }

  private async resolveResourceConflict(collaboration: AgentCollaboration, conflict: any): Promise<string> {
    // 协商资源分配
