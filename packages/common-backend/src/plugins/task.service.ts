...
export class TaskService {
  ...
  async createTask(data: {
    ...
    // 自动尝试分配任务
    if (!data.parentTaskId) { // 只为根任务自动分配
   ...
   * 自动分配任务
   */
  async autoAssignTask(taskId: string, options: TaskSchedulingOptions = {}): Promise<TaskAssignment[]> {
   ...
   * 获取Agent的任务列表
   */
  async getAgentTasks(agentId: string, filters?: {
    status?: TaskStatus[];
    limit?: number;
    offset?: number;
  }): Promise<{ tasks: Task[]; total: number }> {
