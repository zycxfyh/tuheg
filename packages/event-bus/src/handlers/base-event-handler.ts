...
import { type IEvent, type IEventHandler, IHandlerExecutionContext } from '../interfaces'
 ...
 */
@Injectable()
export abstract class BaseEventHandler<TEvent extends IEvent = IEvent>
  implements IEventHandler<TEvent>
{
  ...
  async handle(event: TEvent): Promise<void> {
    ...
    try {
      ...
      const duration = Date.now() - startTime.getTime()
      this.logger.error(`Event processing failed: ${event.eventType} (${duration}ms)`, {
        eventId: event.eventId,
        correlationId: event.correlationId,
        error: error.message,
        stack: error.stack,
 ...
 * 对于简单的处理逻辑，可以直接继承此类
 */
export abstract class SimpleEventHandler<
