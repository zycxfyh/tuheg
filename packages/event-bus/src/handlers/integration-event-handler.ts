...
import type { IIntegrationEvent } from '../interfaces'
...
export abstract class IntegrationEventHandler<
  TEvent extends IIntegrationEvent = IIntegrationEvent,
 ...
 */
@Injectable()
export abstract class ExternalServiceHandler<
  TEvent extends IIntegrationEvent = IIntegrationEvent,
> extends IntegrationEventHandler<TEvent> {
  ...
  protected async process(event: TEvent): Promise<void> {
    ...
    if (!(await this.validateExternalAuth(event))) {
      throw new Error(
        `Authentication failed for external service: ${this.getExternalServiceName()}`
      )
    }
 ...
 */
@Injectable()
export abstract class DeadLetterHandler<
  TEvent extends IIntegrationEvent = IIntegrationEvent,
> extends IntegrationEventHandler<TEvent> {
