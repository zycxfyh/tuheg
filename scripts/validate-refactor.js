...
function checkPackageExists(packageName) {
  ...
  log(
    ...
    `${exists ? '✅' : '❌'} Package ${packageName}: ${exists ? 'exists' : 'missing'}`
  )
  return exists
}
...
function checkPackageJson(packageName) {
  ...
  try {
    ...
    log(
      hasName && hasMain ? colors.green : colors.yellow,
...
function checkIndexFile(packageName) {
  ...
  log(
    exists ? colors.green : colors.red,
    `${exists ? '✅' : '❌'} ${packageName}/src/index.ts: ${exists ? 'exists' : 'missing'}`
  )
  return exists
}
...
function main() {
  ...
  if (fs.existsSync(nxJsonPath)) {
    try {
      ...
      log(
        hasNewProjects ? colors.green : colors.red,
